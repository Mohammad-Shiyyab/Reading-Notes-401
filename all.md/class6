...
so if we have a list of 10 items and we want to find the item in the list, we have to go through all the items in the list, so the time complexity is O(n). but if we want to get item in a array of 100 items,the size of array will not effect the time complexity because we know the index of the item we want to get, so the time complexity is O(1).
